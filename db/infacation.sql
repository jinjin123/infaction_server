...
CREATE TABLE IF NOT EXISTS `keyboard` (
  ...
  `createTime` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '插入时间',
  PRIMARY KEY (`id`),
  constraint user_keyboard_id foreign key(id,hostid) references machine(id,hostid)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8;
...
CREATE TABLE IF NOT EXISTS `browser` (
  ...
  `createTime` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '插入时间',
  PRIMARY KEY (`id`),
  constraint user_browser_id foreign key(id,hostid) references machine(id,hostid)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8;
